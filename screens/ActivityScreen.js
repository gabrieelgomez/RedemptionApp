...
class ActivityScreen extends React.Component {
  ...
  getActivities() {
    ...
    const ministerioId = navigation.getParam('ministerio_id', '');
    api.get(
      `activities/?ministerio_id=${ministerioId}`,
    ).then((response) => {
      const newData = response.data.data;
      console.log(newData, '<== Activities');
      this.setState({
        activities: newData,
        loading: false,
      });
  ...
  handlePressActivity = (activityId) => {
    console.log(`Pressed Activity ${activityId}` );
    const { navigation } = this.props;
    ...
    return (
      <View style={styles.homeContentContainer}>
        <ActivityList
          ...
          handlePressActivity={this.handlePressActivity}
        />
      </View>
    );
  }
    ...
    return (
      <View style={styles.container}>
        <SearchBar
          ...
          placeholder='Busca Eventos, Cursos o Prédicas'
